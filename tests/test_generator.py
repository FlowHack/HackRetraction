...
def test_start_movement_position() -> None:
    # dx=320 -> рафт стартует с (320/2-35, 320/2-38) = (125.0, 122.0).
    gcode = _gcode()
    assert "G1 F9000 X125.00 Y122.00 Z0.20" in gcode
...
def test_raft_extrusion_values() -> None:
    # eValue(70)*1.25 = 2.01604 (первая линия слоя 1, шаг 1 мм)
    gcode = _gcode()
    assert "G1 F2100 X195.00 Y122.00 E2.01604" in gcode
    # рабочий переход по краю слоя 1: 1 мм вдоль Y, eValue(1)*1.25 = 0.0288
    assert "G1 F2100 X195.00 Y123.00 E2.04484" in gcode
    # вторая линия слоя 1 — Y123 (шаг 1 мм), E накоплен: 2.01604+0.0288+2.01604
    assert "G1 F2100 X125.00 Y123.00 E4.06088" in gcode
...
def test_front_label_printed() -> None:
    ...
    i3 = next(
        ...
        if l.startswith("G0 F9000 X131.00 Y129.00")
    )
    ...
    # наружный контур X131.2, внутренний X130.8
    assert "G0 F9000 X131.20 Y129.00" in gcode
    ...
    assert "G1 F2100 X131.20 Y124.10 E0.11290" in gcode
    assert "G1 F2100 X130.80 Y124.10 E0.00922" in gcode
    assert "G1 F2100 X130.80 Y129.00 E0.11290" in gcode
    ...
    # -> abs (131,126.55)->(134.5,126.55), наружный контур Y126.75, внутренний Y126.35
    assert "G0 F9000 X131.00 Y126.75" in gcode
    assert "G1 F2100 X134.50 Y126.75 E0.08064" in gcode
